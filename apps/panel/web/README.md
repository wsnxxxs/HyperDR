...
- 顶栏「打开照片」旁提供「LUT 库」，打开后右栏切换为独立风格库，中央照片继续预览。支持按名称搜索、多文件导入、点选预览和移出库，选中风格有明确标记；「继续调色」返回强度调整，Escape 或右上角关闭按钮返回原调整位置。没有照片时也可导入收藏，随后打开照片试用。空间设置默认折叠，初次导入按 sRGB 处理，可按作者说明修改；库会记住这些空间设置。库保存在工作目录的 `lut-library/`，换图、服务重启和照片会话过期不会清除。应用到照片时复制为会话资源，移出库不影响该照片的撤销或历史导出。
