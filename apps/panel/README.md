...
- **Windows**：双击项目根目录的 `启动界面.bat`，或在终端运行：
  ```
...
- **导出后校验**——它不是偏好。转换器在写盘*之前*把刚编码的字节解码回来验一遍，
  ...
  增益图引用写坏的错误，所以始终开启。
- **画面风格（渲染器）**——`neutral` 是转换器的旧渲染器：它忽略对比度、鲜艳度和 pop，
  把余量静默钳到 3 stops，而 `/api/curve` 对它返回的是 photographic 的曲线，
  预览会和导出对不上。面板固定跑 `--look photographic`，需要旧渲染器请用命令行。
...
- RAW 预览由转换器通过 LibRaw 做固定半尺寸解码，再按设备限制到最长边 960 或
  1280 像素；它不使用相机内嵌 JPEG，因此高光恢复设置会真实反映在预览中。
