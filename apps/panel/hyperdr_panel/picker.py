...
from .formats import picker_patterns
...
def run_picker(kind: str) -> int:
    ...
    if kind == "folder":
        ...
        path = filedialog.askopenfilename(
            ...
            filetypes=[
                # Built from the converter's own list, both cases, because the
                # Windows dialog matches these case-sensitively and this used
                # to be a hand-written string that a new format had to be
                # remembered into.
                ("Supported images", picker_patterns()),
                ("All files", "*.*"),
            ],
        )
