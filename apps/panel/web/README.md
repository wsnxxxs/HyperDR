...
- 右栏在画面调整上方提供「仅调色 / HDR 增强」，空白页也可选择。仅调色将 LUT 面板置顶，隐藏 HDR/AI 控件，预览与「保存 JPEG」统一使用 SDR 输出；来回切换保留调色和 HDR 参数，重置不改变模式。模式沿用「记住输出设置」偏好。
- 顶栏「LUT 库」打开与设置同风格的独立管理弹窗，支持搜索、多文件导入和移出库；这些操作不选择 LUT，也不改变照片。右栏仅通过「选择 LUT 风格」下拉框应用风格并调整强度。弹窗支持 Escape、遮罩和关闭按钮退出，并保持右栏布局与滚动位置。没有照片时也可管理库。空间设置默认折叠，初次导入按 sRGB 处理，可按作者说明修改；库会记住这些空间设置。库保存在工作目录的 `lut-library/`，换图、服务重启和照片会话过期不会清除。应用到照片时复制为会话资源，移出库不影响该照片的撤销或历史导出。
...
- 直方图在同一张图中叠加原图与当前效果：原图为填充面积加虚线轮廓，当前效果为清晰实线；仅保留亮度 / RGB 切换、图例和明暗刻度。
