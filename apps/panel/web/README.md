...
- 「浏览 LUT 库」在右栏展开「我的 LUT」，支持多文件导入、点选预览、启用/停用和移出库。空间设置默认折叠，初次导入按 sRGB 处理，可按作者说明修改；库会记住这些空间设置。库保存在工作目录的 `lut-library/`，换图、服务重启和照片会话过期不会清除。应用到照片时复制为会话资源，移出库不影响该照片的撤销或历史导出。
