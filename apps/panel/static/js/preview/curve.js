...
const keyOf = (state) => [
  state.encoding, state.look, state.contrast,
  state.hdrRange, state.expansionStart, state.areaCoverage,
].join("\0");

export function createCurve(store, { onChange }) {
   ...
   * moving --gain-strength or dragging the viewer leaves every one of the six
   * inputs above untouched, yet the old code still walked 257 entries through
   * `Math.pow` on each frame. The stamp records both the settings and which
   * source filled the table, so a refetch landing under an unchanged key still
   * invalidates. */
  ...
  function schedule() {
    clearTimeout(timer);
    timer = setTimeout(fetchCurve, 150);
  }
